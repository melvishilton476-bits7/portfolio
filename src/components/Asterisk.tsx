 ...
 *
 * By default the mark is a solid fill. Pass `outline` to draw it as a stroke of
 * the same colour instead (hollow splat), and `boilId` to give that stroke the
 * roughen-edges "boil" shared with the hands / fireplace: an feTurbulence →
 * feDisplacementMap filter whose `seed` steps through a shuffled list (~7fps),
 * so the outline wobbles frame-by-frame like the other hand-drawn artifacts.
 * `boilId` must be unique per instance (filter ids are document-scoped). The
 * `.asterisk-boil` class lets globals.css drop the filter under
  ...
  return (
    ...
    >
      {boilId ? (
        <>
          <defs>
            <filter id={boilId} x="-20%" y="-20%" width="140%" height="140%">
              <feTurbulence
                type="fractalNoise"
                baseFrequency="0.06"
                numOctaves={2}
          ...
          <g className="asterisk-boil" filter={`url(#${boilId})`}>
            {path}
          </g>
        </>
      ) : (
